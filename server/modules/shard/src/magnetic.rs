...
///
/// Filters: card must be in the caller's soul inventory bucket
/// (`owner_id == caller's soul_card_id` under the post-flag-20
/// card-owner model), not the anchor, not already pulled, not
/// flagged with `slot_hold` / `magnetic_hold` / `dead`, and its
/// definition must satisfy the entity with non-zero specificity.
///
/// Uses the `owner_id` btree index — only this soul's version rows
...
fn find_match(
    ...
    // The magnetic anchor lives in the action's soul context — walk
    // up from it to find the soul whose inventory we're pulling
    // from. World-owned anchors (no soul in their chain) yield no
    // candidates.
    let Some(anchor_soul_card_id) = cards::owning_soul(ctx, args.anchor_card_id) else {
        return Ok(None);
    };
    // Enumerate distinct card_ids for this soul once via the
    // btree index — version-row dedupe happens implicitly via the
    // BTreeSet.
    let mut card_ids: BTreeSet<u32> = BTreeSet::new();
    for c in ctx.db.cards().owner_id().filter(anchor_soul_card_id) {
    ...
    for id in card_ids {
        ...
        if card.owner_id != anchor_soul_card_id {
            continue;
        }
...
fn commit_success(
    ...
    // Resolve has-predicates against the relevant souls' stacks.
    // Empty for inner recipes that don't declare has. Walks
    // `owner_id` from each role card up to the first soul
    // (`FLAG_OWNED_BY_PLAYER` set); world-owned roles yield `0`,
    // which disables has-matching for that role (empty stack).
    //
    // For a `root_id == 0` inner recipe, walk up from the anchor
    // instead — the anchor's soul is the conceptual root context
    // for a rootless magnetic action.
    let root_soul_card_id = if root_id != 0 {
        cards::owning_soul(ctx, root_id).unwrap_or(0)
    } else {
        cards::owning_soul(ctx, args.anchor_card_id).unwrap_or(0)
    };
    ...
    let actor_soul_card_id = cards::owning_soul(ctx, actor_for_has).unwrap_or(0);
...
fn commit_failure(
    ...
    // Resolve has-predicates against the anchor's soul.
    let anchor_soul_card_id = cards::owning_soul(ctx, args.anchor_card_id).unwrap_or(0);
    let has_matches = crate::recipe_eval::resolve_has(
        ctx,
        &failure_recipe.id,
        &failure_recipe.has,
        ...
        /* root_soul_card_id  */ 0,
        /* actor_soul_card_id */ anchor_soul_card_id,
        commit_at,
    )?;
...
fn release_pulled_to_inventory(ctx: &ReducerContext, pulled_cards: &[u32]) {
    ...
    for &card_id in pulled_cards {
        cards::update_with(ctx, card_id, |c| {
            ...
            c.flags = cards::decrement_position_hold_count(c.flags);
            // Reset to inventory-loose: macro_zone = owner_id (the
            // soul's card_id under the post-flag-20 model — also the
            // soul-inventory bucket address), surface = 1, micro_zone
            // = state-0 LOOSE, micro_location = 0 (client's mirror
            // loose-preserve gate will keep whatever local xy the
            // player sees — this server reset is mostly to clear the
            // OnRoot chain reference to a dying anchor).
