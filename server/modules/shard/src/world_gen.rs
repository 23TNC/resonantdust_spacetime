...
// card rows. An older draft used per-hex card spawns for these (the
// type slot at id 8 has since been repurposed to `mini_zone`); we
// express the same scenery as a tile byte, which keeps zone data
// dense (1 byte per hex) and avoids the per-card overhead for static
// scenery.
//
