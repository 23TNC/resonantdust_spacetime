...
| `micro_zone`        | u8   | World cards: in-zone hex coords `[local_q:u3][local_r:u3][reserved:u2]`. Inventory cards: held at 0. |
| `micro_location`    | u32  | World cards: variant per `stack_state` in `flags` — parent `card_id` or packed `(i16 x, i16 y)` pixel coords. Inventory cards: held at 0. |
...
| `flags`             | u8   | `[stack_state:u2][reserved:u6]` (bits 7..6 stack_state). World cards: authoritative stack_state. Inventory cards: held at 0. |
...
| `micro_zone`      | u8     | In-zone hex position of the soul.                                                                       |
...
- **Position fields**: `micro_zone`, `micro_location`, `flags`. Will return
  with the world board.
...
- [src/definitions.rs](src/definitions.rs) — `CardDefinition` and `Aspect` registries built lazily from `data/card_types.json`, `data/aspects.json`, and `data/cards/*.json`. Card lookup: `decode_definition(packed) -> Result<Option<&'static CardDefinition>, String>` (by packed u16) and `find_packed(path) -> Result<u16, String>` (by `"type/key"` or `"type/category/key"` string). Aspect side: `aspect_id(name)` / `aspect(id)` / `aspects()`, all `Result`-returning. Aspects are 1-indexed (`ASPECT_NONE = 0`) and `CardDefinition.aspects` carries `(AspectId, i32)` pairs — names are translated to ids at registry build time. Each registry is stored as `OnceLock<Result<Registry, String>>`, so a build failure (malformed JSON, unknown aspect referenced from a card, id out of range, invalid hex color in `style`, duplicate aspect on a single card) is recorded once and returned to every subsequent lookup rather than re-panicking.
