...
| `micro_zone`        | u8   | World cards: `[local_q:u3][local_r:u3][stack_state:u2]` — in-zone hex coords plus the card's stack role (bits 1..0). Inventory cards: defaults to 0 — `insert_card_row` zeroes both this and `micro_location` so the client's "is the server forcing a position?" check (`micro_zone & 0xE0 != 0`, i.e. any bit in `local_q`) reads as no. Code that *does* want to force an inventory placement must go through `insert_card_row_at_position` and set `local_q != 0`. |
| `micro_location`    | u32  | World cards: variant per `micro_zone`'s `stack_state` — parent `card_id` or packed `(i16 x, i16 y)` pixel coords. Inventory cards: 0 by default; non-zero values are only meaningful alongside a non-zero `micro_zone.local_q` (see above). Indexed (btree) so the magnetic chain walk in `magnetic::find_attached` finds a card's children in O(matches + log n). |
...
| `flags`             | u8   | Bit flags for per-card state that doesn't fit in the other fields. Specific bits are assigned as features need them; `insert_card_row` zero-initializes the field, so callers that don't need flags don't have to think about them. |
...
- [src/lib.rs](src/lib.rs) — module root; declares `actions`, `cards`, `definitions`, `magnetic`, `packing`, `players`, `zones`, and (behind the `debug` cargo feature) `debug`.
...
- [src/cards.rs](src/cards.rs) — `cards` table, the `insert_card_row` chokepoint helper (validates layer + target-player existence; zeroes `micro_zone` and `micro_location` so inventory inserts default to "client owns layout"), the `insert_card_row_at_position` opt-in for the rare server-forces-placement case (rejects layer-1 inserts with `local_q == 0` so a caller who thinks they're forcing a placement can't accidentally produce a row the client will silently ignore), the `MICRO_ZONE_LOCAL_Q_MASK` constant (`0xE0`) backing the client's "trust this position" signal, the `InventoryStack` parameter type, the `submit_inventory_stacks` reducer, and `MAX_STACK_BRANCH` / `MAX_STACKS_PER_SUBMISSION` bounds.
...
- [src/definitions.rs](src/definitions.rs) — `CardDefinition`, `Aspect`, and `RecipeDef` registries built lazily from `data/card_types.json`, `data/aspects.json`, `data/cards/*.json` (+ `cards/id.json` for stable `definition_id`s), and `data/recipes/*.json` (+ `recipes/id.json` for stable recipe IDs). Card lookup: `decode_definition(packed)`, `find_packed(path)`, `find_packed_by_key(key)` (O(log n) via the `by_key` map populated from `cards/id.json`). Aspect side: `aspect_id`, `aspect`, `aspects`. Recipe side: `recipe(index)`, `find_recipe(id)`, `recipes_of_type(rt)`. The `Entity` enum carries five leaf forms — `Card(String)`, `Aspect(AspectId, i32)`, `Type(u8)` (resolved from `"@<type_name>"` at parse time), `Any` (wildcard from `"any"`), plus the `And` / `Or` / `WeightedOr` composites. `RecipeDef` carries an optional `tile: Option<Entity>` field for forward-looking world-layer tile preconditions (parsed today, scored 0 by the matcher until tile context is wired up), and an optional `magnetic: Option<MagneticDirection>` field that flips the recipe into "server pulls inputs from the player's inventory" mode (see `magnetic.rs`) — independent of `recipe_type`, so e.g. an `OnCreate` recipe can be magnetic without needing a separate type. `is_hex_type(type_id)` returns whether a card_type's shape (from `card_types.json`) is `"hex"`, used by `magnetic.rs` to decide whether `slot[0]` attaches as a hex root or stacks top/bottom. All lookups return `Result<…, String>`. Each registry is stored as `OnceLock<Result<Registry, String>>`, so a build failure (malformed JSON, unknown aspect, unknown card type, missing entry in `id.json`, invalid hex color, duplicate aspect on a card, unknown recipe type, malformed entity grammar, missing duration fallback, …) is recorded once and returned to every subsequent lookup rather than re-panicking.
- [src/magnetic.rs](src/magnetic.rs) — `magnetic_inputs` (private, scheduled by `Interval`) plus the `magnetic_tick` reducer; runs the server-driven slot-fill loop for any action whose recipe carries a `magnetic: "top" | "bottom"` field (the field rides alongside the existing `recipe_type` — currently only `OnCreate` recipes use it, but the lifecycle plumbing is type-agnostic). Each row replicates `action_id` and tracks up to `MAGNETIC_MAX_SLOTS` (5) input card_ids in `slot_1..slot_5`. Per tick: walk what's already attached to the actor (using `STACK_STATE_HEX_ROOT == 3` for slot 0 when the actor is hex-shaped per `definitions::is_hex_type`, else top/bottom per `recipe.magnetic`), stamp `cards.flags::position_held` and a `CardHold` on each, then top up from the player's inventory (`owner_id == action.owner_id`, `layer == LAYER_INVENTORY`) by walking the indexed owner_id rows for the first non-blocked card that satisfies the next slot's `Entity` (skipping the actor, earlier filled slots, already-`position_held` cards, and cards held by *any* `CardHold`). Placement writes `local_q = 1` into `micro_zone`'s high 3 bits so the client's "trust this position" check picks it up, **and** promotes the placed card's `layer` to the actor's layer — so a card scooped from inventory (layer 1) onto a world tile (layer 64) lands on the world layer alongside the rest of the chain. Chain walk uses the `cards.micro_location` btree index for O(matches + log n) child lookup. `install` is called from `actions::start_action` when `recipe.magnetic.is_some()`; `release` is called from `actions::delete_action_rows` and clears `position_held` plus deletes the schedule (CardHold cleanup is the regular path's job). Stack-state constants (`STACK_STATE_LOOSE`, `STACK_STATE_TOP`, `STACK_STATE_BOTTOM`, `STACK_STATE_HEX_ROOT`) live here and must be kept in lockstep with the client's `cardData.ts`.
- [src/zones.rs](src/zones.rs) — `zones` table for bulk world-tile storage; one row per 8×8 chunk. Each row carries a shared `packed_definition: u8` (`[card_type:u4][card_category:u4]` — the high byte of every cell's full `u16 packed_definition`) plus `packed_definition_ids: Vec<u64>` (8 entries × 8 packed `u8` `definition_id`s = 64 cells, byte-packed low-byte-first, row-major: `cell_index = local_r * 8 + local_q`). `definition_id == 0` is the empty-cell sentinel. Helpers: `LocalCoord` (zone-local `(q, r)` with `from_micro_zone` / `to_micro_zone` round-trips through `Card.micro_zone`'s high 6 bits), `read_cell` / `write_cell` (raw bit math), `read_cell_checked` / `validate_layout` (length-checked wrappers), `cell_packed_definition(zone_byte, id)` (reassemble the full `u16`), and `insert_empty_zone` / `lookup_cell` / `set_cell` for table-level CRUD. No reducers — same authority model as `actions.rs` and `cards.rs`: client-driven changes happen through validated reducers in other modules calling these helpers, never a direct `set_cell` reducer that a client could spam. The compression is a 64× win on row count for dense world tiles; per-tile `Card` rows are reserved for tiles that need first-class identity (e.g. trees being chopped).
- [src/debug.rs](src/debug.rs) — development-only reducers and the underlying spawn helpers shared with the bootstrap path. Bypasses authentication; should not ship to production. The card-creation chokepoint (`cards::insert_card_row`) and key-resolution layer (`definitions::find_packed_by_key`) still run, so a typo can't slip a bogus row through. Public surface: (a) `spawn_card_for_player(ctx, player_id, card_key) -> Card` — shared helper that resolves `card_key` against `cards/id.json` and inserts an inventory card owned by `player_id`; (b) `debug_spawn(player_id, card_key)` — a thin reducer wrapper for one-off CLI/explorer dev spawns; (c) `bootstrap()` — reducer that reads embedded `data/bootstrap/bootstrap.json` and seeds the database. Bootstrap creates any missing players (idempotent — find by `name_normalized`, create if absent), replaces zones at each `(zone_q, zone_r)` (zone `macro_zone` packed via `pack_world_macro_zone`; t0..t7 written verbatim into `Zone.packed_definition_ids`; bootstrap's `layer` field on a zone is read but not stored, since the Zone schema today identifies a chunk by `macro_zone` alone), and adds the cards in the JSON's `card` array to the first player's inventory (additive — re-running bootstrap appends another set).
