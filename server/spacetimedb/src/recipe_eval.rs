...
///
/// Implementation: scan cards owned by `owner_id` (the soul's
/// player), build a parent → children map keyed on `micro_location`,
/// then BFS from `soul_card_id` with a depth cap. Cards with `dead`
/// or `slot_hold` set are excluded — slot_held cards are claimed by
/// an in-flight recipe and must not be re-bound to another action's
/// has-predicate concurrently. Cards not in a chain state (`Free` /
/// `OnHex`) are excluded — only `OnRoot` (immediate stacked-on-root
...
pub fn soul_stack(
    ...
    if soul_card_id == 0 || owner_id == 0 {
        return Vec::new();
    }
    // Build parent → children map from this player's owned cards.
    // Iterating owner_id is btree-index-backed; the per-card_id
    // `cards::latest` lookups dedupe history rows naturally.
    let mut seen: BTreeSet<u32> = BTreeSet::new();
    let mut children_of: BTreeMap<u32, Vec<Card>> = BTreeMap::new();
    for row in ctx.db.cards().owner_id().filter(owner_id) {
...
fn bind_has_role(
    ...
    if owner_id == 0 {
        return Err(format!(
            "recipe {recipe_id}: {field_label}.{role} requires owner_id > 0 (world owner has no soul)"
        ));
    }
    let player = crate::players::latest(ctx, owner_id).ok_or_else(|| {
        format!("recipe {recipe_id}: {field_label}.{role}: player {owner_id} not found")
    })?;
    if player.soul_card_id == 0 {
        return Err(format!(
            "recipe {recipe_id}: {field_label}.{role}: player {owner_id} has no soul card"
        ));
    }
    let stack = soul_stack(ctx, player.soul_card_id, owner_id, direction);
            ...
            None => {
                return Err(format!(
                    "recipe {recipe_id}: {field_label}.{role}[{i}] not satisfied — \
                     no matching card in player {owner_id}'s soul stack"
                ));
            }
        }
    }
